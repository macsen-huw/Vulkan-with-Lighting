...
InputModel load_compressed_wavefront_obj( char const* aPath )
{
	...
	// Ask rapidobj to load the requested file, plain OBJ only
...
#	endif
	...
	// Ask rapidobj to load the requested file, optionally ZSTD compressed OBJ
	// Using std::istream as input requires us to specify directory of the
	// model separately, which is where rapidobj should find the associate
	...
	std::unique_ptr<std::istream> ins = [&] () -> std::unique_ptr<std::istream> {
		auto const* last = std::strrchr( aPath, '-' );
		if( last && 0 == std::strcmp( last+1, "zstd" ) )
			return std::make_unique<ZStdIStream>( aPath );

		return std::make_unique<std::ifstream>( aPath );
	}();
